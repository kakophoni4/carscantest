...
По умолчанию парсит 15 страниц (~450 авто). Чтобы спарсить больше, поменяй `SCRAPER_MAX_PAGES` в `docker-compose.yml`:

```yaml
# worker service
SCRAPER_MAX_PAGES: "50"   # ~1500 авто
```

Для полного сайта (~5000+ страниц) можно поставить большое число, но парсинг займёт несколько часов из-за детальных страниц и автоперевода. Повторные запуски обновляют существующие записи, а не дублируют.

Интервал между запусками задаётся через `SCRAPER_INTERVAL_MINUTES` (по умолчанию 60 минут).
